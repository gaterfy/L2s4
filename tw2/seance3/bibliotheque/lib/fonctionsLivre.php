<?php
//We import the comparison functions
require_once('fonctionsComparaison.php');
...
*
* @param file $file The file describing the books
* @return array an array containing all the books
*/
...
/**
* Return the HTML Code of a library
*
* @param array $liste the array containing all the books
* @return string the HTML Code corresponding to the library of books
*/
function biblioToHTML($liste, $sort="none") {
  if ($sort === "titles") {
    usort($liste, 'compareBooks');
  } elseif ($sort === "categories") {
    usort($liste, 'compareBooksCategories');
  } else {
    if ($sort != "none") {
      throw new Exception("Invalid parameter for $sort");
    }
  }
  $HTMLCode = '';
  foreach ($liste as $book) {
    $HTMLCode .= bookToHTML($book);
  }
  return $HTMLCode;
}
?>
